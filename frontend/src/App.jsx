...
const App = () => {
  ...
  const handleSetBackup = async () => {
    ...
    setState(prev => ({ ...prev, loading: true }));
    try {
      const result = await state.actor.setBackup(backupWallet);
      if ('ok' in result) {
        setSuccess(result.ok);
        setBackupWallet('');
        await loadUserData(state.actor);
  ...
  const handleTransfer = async () => {
    ...
    try {
      const result = await state.actor.icrc1_transfer({
        to: { owner: transferTo, subaccount: [] },
        amount: amountNat,
        fee: [tokenInfo.fee],
        memo: [],
        from_subaccount: [],
                    ...
                    <div className="flex items-center">
                      ...
                      <div>
                        ...
                        <p className="font-mono text-sm text-gray-900">
                          {userStatus.backupWallet || 'Not configured'}
                        </p>
                      </div>
                    </div>
                    {userStatus.backupWallet && (
                      <Button 
                        onClick={() => copyToClipboard(userStatus.backupWallet)} 
                        variant="ghost" 
                        size="sm"
                        icon={Copy}
                      >
